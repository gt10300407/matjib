...
def test_mobile_motion_is_minimized():
    text = (ROOT / "frontend" / "app-mobile.js").read_text(encoding="utf-8")
    assert "globalTimeline.timeScale(mq.matches ? 1000 : 1)" in text
...
def test_mobile_map_scaling_is_applied_without_extra_animation_frame():
    text = (ROOT / "frontend" / "app-mobile.js").read_text(encoding="utf-8")
    korea_wrapper = text.split("drawKorea = function responsiveDrawKorea", 1)[1].split("return result;", 1)[0]
    province_wrapper = text.split("drawProvince = function responsiveDrawProvince", 1)[1].split("return result;", 1)[0]
