...
def get_api_key_status(env_path: Path):
    ...
    for logical_name, env_name in ENV_KEYS.items():
        ...
        result[logical_name] = {"configured": bool(value), "masked": mask_key(value)}
    return result


def save_api_keys(env_path: Path, kakao: str | None, data_go: str | None, tourapi: str | None, google: str | None = None):
    supplied = {
...
def migrate_api_keys_from_sibling_versions(project_root: Path, persistent_env: Path):
    ...
    missing = [env_name for env_name in ENV_KEYS.values() if not persistent.get(env_name, "").strip()]
    if not missing:
