...
def _is_verified(rating: float, reviews: int) -> bool:
    # Precision-first rule: 적은 리뷰의 5.0보다 충분한 평가가 쌓인 고평점 매장을 우선한다.
...
class GooglePlacesCollector:
    ...
    async def _search_one(self, client: httpx.AsyncClient, province: str, city: str, bbox, spec):
        ...
        body: dict[str, Any] = {
            "textQuery": query_tpl.format(city=city),
            "languageCode": "ko",
            "regionCode": "KR",
        ...
        for p in r.json().get("places", []):
            ...
            if p.get("businessStatus") not in (None, "OPERATIONAL"):
                continue
            if not _is_verified(rating, reviews):
                continue

            types = list(p.get("types") or [])
            cuisine = _classify(types, category)
            loc = p.get("location") or {}
            score = _bayesian_score(rating, reviews)
            out.append({
                "provider": "google",
    ...
    async def collect_verified(self, province: str, city: str, bbox=None):
        if not self.enabled:
            return [], {"candidate_count": 0, "verified_count": 0, "api_calls": 0}
        ...
        for batch in batches:
            ...
            for row in batch:
                ...
                if not pid:
                    continue
                if pid in found:
                    found[pid]["query_hits"] = int(found[pid].get("query_hits", 1)) + 1
                    # 구체적인 cuisine 분류를 우선한다.
                    if found[pid].get("cuisine") in {"기타", "전체"} and row.get("cuisine") not in {"기타", "전체"}:
                        found[pid]["cuisine"] = row["cuisine"]
                        found[pid]["category"] = row["cuisine"]
                else:
                    found[pid] = row

        rows = list(found.values())
        rows.sort(key=lambda x: (x.get("taste_score", 0), math.log10((x.get("user_rating_count") or 0) + 1), x.get("query_hits", 1)), reverse=True)
        return rows, {
