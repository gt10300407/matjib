...
def _classify(types: list[str], query_category: str) -> str:
    ...
    if query_category != "전체":
        return query_category
    return "기타"


def _grid_circles(bbox, n: int = 2):
    """Cover bbox with bounded circles for Nearby popularity discovery.

    Default 2x2 is deliberately conservative on Google billable request count.
    It is generic for every city and does not contain region/store exceptions.
    """
    minx,miny,maxx,maxy=map(float,bbox)
    dx=(maxx-minx)/n
    dy=(maxy-miny)/n
...
class GooglePlacesCollector:
    ...
    def _headers(self):
        return {"X-Goog-Api-Key":self.key,"X-Goog-FieldMask":FIELD_MASK,"Content-Type":"application/json"}

    def _normalize_place(self,p:dict,province:str,city:str,query_category:str,query_text:str,discovery_mode:str):
        if p.get("businessStatus") not in (None,"OPERATIONAL"):
            return None
        rating=float(p.get("rating") or 0)
        reviews=int(p.get("userRatingCount") or 0)
        types=list(p.get("types") or [])
        cuisine=_classify(types,query_category)
        ...
        return {
            ...
            "query_category":query_category,"query_text":query_text,"query_hits":1,"discovery_mode":discovery_mode,"raw_json":p,
        }
    ...
    async def _search_one(self,client:httpx.AsyncClient,province:str,city:str,bbox,spec):
        ...
        for p in data.get("places",[]):
            row=self._normalize_place(p,province,city,category,query,"keyword")
            if row:
                out.append(row)
        return out
    ...
    async def _nearby_one(self,client:httpx.AsyncClient,province:str,city:str,circle,group:str):
        ...
        out=[]
        for p in data.get("places",[]):
            row=self._normalize_place(p,province,city,"카페" if group=="카페" else "전체",f"{city} {group} 인기도 주변검색","nearby_popularity")
            if row:
                out.append(row)
        return out
