...
class TasteStore:
    ...
    def _values(province: str, city: str, r: dict, stamp: str):
        return (
            ...
            city,
            r.get("name") or "이름없음",
            r.get("cuisine") or r.get("category") or "기타",
            r.get("primary_type"),
            r.get("road_address") or r.get("address"),
            r.get("x"),
            r.get("y"),
            r.get("place_url"),
            float(r.get("rating") or 0),
            int(r.get("user_rating_count") or 0),
            float(r.get("taste_score") or 0),
            int(r.get("query_hits") or 0),
            json.dumps(r.get("raw_json") or {"evidence": r.get("evidence") or {}}, ensure_ascii=False),
            stamp,
        )
        ...
        for r in rows:
            values.append(
                (
                    ...
                    city,
                    r.get("name") or "이름없음",
                    r.get("category") or r.get("cuisine") or "일반음식점",
                    r.get("road_address") or r.get("address"),
                    r.get("phone"),
                    r.get("status"),
                    json.dumps(r.get("raw_json") or {}, ensure_ascii=False),
                    stamp,
                )
            )
        with self.connect() as con:
            ...
            if values:
                con.executemany(
                    ...
                    INSERT INTO public_restaurant_inventory(
                      provider_id,province,city,name,category,address,phone,status,raw_json,fetched_at
                    ) VALUES(?,?,?,?,?,?,?,?,?,?)
                    """,
                    values,
                )
    ...
    def get_public_inventory(self, province: str, city: str, limit: int = 20000):
        with self.connect() as con:
            rows = con.execute(
                """
                SELECT provider_id,province,city,name,category,address,phone,status,raw_json,fetched_at
    ...
    def search_public_inventory(self, province: str, city: str, q: str, limit: int = 30):
        like = f"%{q.strip()}%"
        with self.connect() as con:
            rows = con.execute(
                """
    ...
    def public_inventory_count(self, province: str, city: str) -> int:
        with self.connect() as con:
            return int(
                con.execute(
                    "SELECT COUNT(*) FROM public_restaurant_inventory WHERE province=? AND city=?",
                    (province, city),
                ).fetchone()[0]
            )
    ...
    def _inventory_row(row):
        ...
        return {
            ...
            "query_text": f"{d['city']} 일반음식점 인허가 명부",
            "query_hits": 1,
    ...
    def _public_row(row):
        ...
        reviews = int(d["user_rating_count"] or 0)
        score = float(d["taste_score"] or 0)
        ...
        return {
            ...
            "status": "추천 맛집",
            "verified_public": bool(evidence.get("official_excellent")),
            "rating": rating,
            "user_rating_count": reviews,
            "taste_score": score,
            "query_hits": int(d["query_hits"] or 0),
