...
def seed_foods_for_region(province: str, city: str):
    return [{"name":x["name"],"subtitle":x.get("subtitle"),"emoji":x.get("emoji"),"source_label":x.get("source_label"),"source_url":x.get("source_url")} for x in REGIONAL_FOODS if x.get("province")==province and x.get("city")==city]
...
async def lifespan(app: FastAPI):
    ...
    if db.last_repair_reason: print(f"[DB] repaired={db.last_repair_reason}")
    if db.last_backup_path: print(f"[DB] backup={db.last_backup_path}")
    yield
...
class ApiKeyClearRequest(BaseModel): source: str
...
def require_local_settings():
    if PUBLIC_MODE: raise HTTPException(status_code=403, detail="공개 서비스에서는 관리자 진단/설정 기능을 사용할 수 없어.")
...
def save_api_key_settings(req: ApiKeySettingsRequest):
    ...
    keys=save_api_keys(CONFIG_ENV_PATH,kakao=req.kakao,data_go=req.data_go,tourapi=req.tourapi)
...
def clear_api_key_setting(req: ApiKeyClearRequest):
    require_local_settings()
    try: keys=clear_api_key(CONFIG_ENV_PATH,req.source)
    except KeyError: raise HTTPException(status_code=400,detail="지원하지 않는 API source야.")
    return {"ok":True,"keys":keys}

@app.get("/api/v1/settings/api-keys/test/{source}")
async def test_single_api_key(source: str):
    require_local_settings()
    if source not in {"kakao","general","excellent","tourapi"}: raise HTTPException(status_code=400,detail="지원하지 않는 API source야.")
...
async def test_api_key_direct(req: ApiKeyTestRequest):
    require_local_settings()
    if req.source not in {"kakao","general","excellent","tourapi"}: raise HTTPException(status_code=400,detail="지원하지 않는 API source야.")
...
def health():
    if PUBLIC_MODE: return {"ok":True,"version":"1.5.0","public_mode":True}
    return {"ok":True,"version":"1.5.0","db":str(db.path),"app_data_dir":str(APP_DATA_DIR),"config_env":str(CONFIG_ENV_PATH),"db_repair_reason":db.last_repair_reason,"db_backup_path":db.last_backup_path,"legacy_migrated_from":LEGACY_DB_MIGRATED_FROM,"key_migration":KEY_MIGRATION,"public_mode":False}
...
def stats(province: str | None = None, city: str | None = None):
    try:
        result=db.get_stats(province=province,city=city);result["storage"]="sqlite";return result
...
def db_repair():
    ...
    db.last_repair_reason="manual repair";db.last_backup_path=str(backup) if backup else None;db.init_schema();db.seed_foods(REGIONAL_FOODS)
...
def db_diagnostics():
    ...
    try:
        with db.connect() as con:
            info["path"]=str(db.path);info["exists"]=Path(db.path).exists();info["tables"]=[r[0] for r in con.execute("SELECT name FROM sqlite_master WHERE type='table' ORDER BY name").fetchall()]
        db.init_schema();info["stats"]=db.get_stats()
    except Exception as exc:
        info["ok"]=False;info["error"]=f"{type(exc).__name__}: {exc}"
    return info

@app.get("/api/v1/region")
def region(province: str, city: str, limit: int = Query(500,ge=1,le=2000)):
    foods=seed_foods_for_region(province,city);storage="sqlite";storage_error=None
    try:
        db_foods=db.get_foods(province,city)
        if db_foods: foods=db_foods
        restaurants=db.get_restaurants(province,city,limit)
    except Exception as exc:
        storage="memory";storage_error=f"{type(exc).__name__}: {exc}";restaurants=refresh_service.get_cached_restaurants(province,city)[:limit]
    cached=refresh_service.get_cached_restaurants(province,city)
    if cached:
        restaurants=cached[:limit]
        if storage=="sqlite": storage="sqlite+memory"
    try: last_refresh=db.get_last_refresh(province,city)
    except Exception: last_refresh=refresh_service.get_cached_meta(province,city)
...
def search(q: str = Query(...,min_length=1),limit: int = Query(200,ge=1,le=1000)):
    try: restaurants=db.search_restaurants(q,limit)
    except Exception: restaurants=[]
    return {"query":q,"restaurants":restaurants}
